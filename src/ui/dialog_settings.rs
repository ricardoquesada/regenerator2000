...
pub fn handle_input(
    ...
    let dialog = &mut ui_state.system_settings_dialog;

    match key.code {
        KeyCode::Esc => {
            if dialog.is_selecting_theme {
                dialog.is_selecting_theme = false;
            } else {
                dialog.close();
        ...
        KeyCode::Enter | KeyCode::Char(' ') => {
            ...
            } else if dialog.selected_index == 0 {
                app_state.system_config.open_last_project =
                    !app_state.system_config.open_last_project;
                let _ = app_state.system_config.save();
            } else if dialog.selected_index == 1 {
                app_state.system_config.sync_blocks_view =
                    !app_state.system_config.sync_blocks_view;
                let _ = app_state.system_config.save();
            } else if dialog.selected_index == 2 {
                dialog.is_selecting_theme = true;
            }
        }
        _ => {}
    }
