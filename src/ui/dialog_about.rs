...
pub fn render(f: &mut Frame, ui_state: &UIState, area: Rect, dialog: &AboutDialog) {
    if !dialog.active {
        return;
    }

    // Only render if we have logo and picker, which seems to be the constraint in original code
    // actually, let's keep the logic close to original
...
pub fn handle_input(key: crossterm::event::KeyEvent, ui_state: &mut UIState) {
    let dialog = &mut ui_state.about_dialog;
