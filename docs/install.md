...
- `--dump-system-config-files <PATH>`: Dump all built-in system config files (`system-*.toml`) to the specified directory and exit. The files can then be edited and placed back in the [app config directory](#system-config-files) to override or extend platform definitions.
...
Regenerator 2000 ships with built-in platform definitions (`system-*.toml`) for every supported system.
You can **override** any of these definitions, or **add entirely new platforms**, by placing custom TOML
