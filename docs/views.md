...
- `j_`: indicating that the label is a jump target (`jmp j_C000`).
- `b_`: indicating that the label is a branch target (`bne b_C000`, `bmi b_C000`, etc.).
- `s_`: indicating that the label is a subroutine target (`jsr s_C000`).
- `f_`: indicating that the label is a field address (`lda f_C000,x`).
- `a_`: indicating that the label is an absolute address (`lda a_C000`).
- `p_`: indicating that the label is a pointer address (`lda (p_A0, x)`).
- `e_`: indicating that the label is an external address (`jmp e_EA31`). External means an address outside the current file.
- `zpf_`: indicating that the label is a zero-page field address.
- `zpa_`: indicating that the label is a zero-page absolute address.
- `zpp_`: indicating that the label is a zero-page pointer address.
