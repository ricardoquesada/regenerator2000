...
use regenerator2000_core::exporter::{verify_all_assemblers, verify_roundtrip};
use regenerator2000_core::state::{AppState, Assembler};
...
fn test_roundtrip_synthetic_code() {
    let mut state = AppState::new();
    state.origin = regenerator2000_core::state::Addr(0x0801);
...
fn test_roundtrip_synthetic_mixed_code_data() {
    use regenerator2000_core::state::BlockType;

    let mut state = AppState::new();
    state.origin = regenerator2000_core::state::Addr(0xC000);
    state.raw_data = vec![
        0xA9, 0x01, // LDA #$01
        0x8D, 0x20, 0xD0, // STA $D020
        0x60, // RTS
...
fn test_roundtrip_synthetic_data_words() {
    ...
    let mut state = AppState::new();
    state.origin = regenerator2000_core::state::Addr(0x0800);
    state.raw_data = vec![
        0xA9, 0x42, // LDA #$42
        0x60, // RTS
        // Word table
